...
from py_goldsrc_demo.exceptions import (
    InvalidMagicException,
    InvalidDemoProtocolException,
    InvalidNetProtocolException,
    InvalidModException,
)
...
from py_goldsrc_demo.macros.base import (
    ClientDataMacro,
    NetMsgMacro,
    DemoBufferMacro,
    ConsoleCommandMacro,
...
class BaseDemoParser:
    ...
    class Directory:
        ...
        def from_stream(binary_stream: BytesIO):
            ...
            flags, cd_track, time, frames, offset, length = unpack_le(
                "IifIII", binary_stream.read(4 + 4 + 4 + 4 + 4 + 4)
            )
            macros = list()
            return BaseDemoParser.Directory(
                dir_id, name, flags, cd_track, time, frames, offset, length, macros
            )
    ...
    class MalformedDirectoryException(Exception):
        ...
        def __init__(self, directory: BaseDemoParser.Directory):
            super().__init__(
                f'Malformed directory "{directory.name}" @ Offset {directory.offset}'
            )
    ...
    def __init__(
        self,
        bs: BytesIO,
        demo_protocol_target: int,
        net_protocol_target: int,
        mod_name_target: str,
    ):
        ...
        for _ in range(directory_count):
            self.directories.append(
                BaseDemoParser.Directory.from_stream(self.binary_stream)
            )
    ...
    def get_macro_by_id(self, base_macro: BaseMacro) -> BaseMacro:
        match base_macro.type:
            ...
            case 3:
                return ConsoleCommandMacro.from_base_macro(
                    base_macro, self.binary_stream
                )
            case 4:
